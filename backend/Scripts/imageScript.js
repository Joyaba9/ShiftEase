...
/**
 * Function to add a profile photo for an business.
 * 
 * @param {number} businessId
 * @param {string} localFilePath - The local file path of the photo.
 * @param {string} originalName - The original file name of the photo.
 * @returns {Promise<object>} - The updated business object with the new profile image URL.
 */
export async function addBusinessPhoto(businessId, localFilePath, originalName) {
    console.log(`Starting addBusinessPhoto for business_id: ${businessId}`);
    console.log(`Local file path: ${localFilePath}`);
    console.log(`Original file name: ${originalName}`);

    
    const fileName = `profile_images/business_${businessId}-${Date.now()}-${originalName}`;
    const bucket = storage.bucket(bucketName);
    const file = bucket.file(fileName);
    ...
    try {
        ...
        // Final update on the businesses table itself
        const updateQuery = `
            UPDATE businesses
            SET profile_image_url = $1
            WHERE business_id = $2
            RETURNING business_id, business_name, profile_image_url;
        `;
        const result = await client.query(updateQuery, [publicUrl, businessId]);

        if (result.rowCount === 0) {
            throw new Error(`Business with business_id ${businessId} not found.`);
        }

        console.log(`Profile photo updated for business_id: ${businessId}`);
        return result.rows[0];
    } catch (error) {
        console.error('Error in addBusinessPhoto:', error);
        throw error;
    } finally {
        if (client) {
            await client.end();
...
/**
 * Retrieves the profile photo URL for an business.
 * 
 * @param {number} businessId
 * @returns {string} The profile photo URL (on Google Cloud Bucket).
 * @throws Will throw an error if the business is not found or there are database issues.
 */
export async function getBusinessPhoto(businessId) {
    console.log(`Fetching profile photo for business_id: ${businessId}`);
